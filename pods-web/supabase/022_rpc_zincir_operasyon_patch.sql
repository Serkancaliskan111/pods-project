-- rpc_is_operasyonel_guncelle + yardımcı fonksiyon (021 ile aynı gövde).
-- PostgreSQL'de CREATE FUNCTION IF NOT EXISTS yoktur; CREATE OR REPLACE = idempotent (aynı script'i tekrar çalıştırabilirsiniz).
-- Tam kurulum: doğrudan güncel 021 dosyasını çalıştırmanız yeterlidir; bu dosya eski 021 sonrası yamayı tek parça yapıştırmak içindir.
begin;
