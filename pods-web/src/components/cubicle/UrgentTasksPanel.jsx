...
import { getTaskDueDate, isCubicleHomeOverdueTask } from '../../lib/cubicleHomeTaskBuckets.js'

function formatUrgentTime(task) {
  const due = getTaskDueDate(task)
  if (!due) return '—'
  return due.toLocaleTimeString('tr-TR', { hour: '2-digit', minute: '2-digit' })
}

function dayProgressForDue(task, now = new Date()) {
  const due = getTaskDueDate(task)
  if (!due) return null
  ...
  if (span <= 0) return 50
  const pct = ((due.getTime() - dayStart.getTime()) / span) * 100
...
export default function UrgentTasksPanel({
  ...
  const timeline = useMemo(
    () =>
      (tasks || []).map((task) => ({
        task,
        timeLabel: formatUrgentTime(task),
        progress: dayProgressForDue(task, now),
          ...
          {!loading && count > 0 ? (
            <>
              ...
              >
                ...
                <div className="relative h-1.5 overflow-hidden rounded-full bg-red-100">
                  {timeline.map(({ task, progress, overdue, timeLabel }) => (
                    <button
                      key={task.id}
                      type="button"
                ...
                <div className="mt-1 flex justify-between text-[9px] font-medium text-red-700/65">
                  <span>00</span>
                  <span>12</span>
                  <span>24</span>
                </div>
              ...
              <ul className="space-y-1.5 p-2.5 sm:p-3">
                {timeline.map(({ task, overdue, timeLabel }) => (
                  <li key={task.id}>
                    <button
                      type="button"
                      onClick={() => onOpenTask?.(task)}
                    ...
                    >
                      <span
                        className={`w-11 shrink-0 text-center text-[11px] font-extrabold tabular-nums ${
