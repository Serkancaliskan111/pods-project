...
export function resolveAuthPermissions(profile, personel) {
  return profile?.yetkiler || personel?.yetkiler || {}
}

/** Şablon oluştururken seçilebilir kapsamlar */
export function allowedTemplateScopesForCreator({
  ...
  const scopes = []
  const canManage = canSeeTaskTemplates(flat, false)
  ...
  if (canManage) {
    ...
    if (unitIds.length > 0) {
      scopes.push(TEMPLATE_KAPSAM.BIRIM)
    }
  }
...
export function kapsamLabel(row, { companyNameById = {}, unitNameById = {} } = {}) {
  const k = row?.kapsam || TEMPLATE_KAPSAM.SIRKET
  if (k === TEMPLATE_KAPSAM.GLOBAL) return 'Global'
  if (k === TEMPLATE_KAPSAM.BIRIM) {
    const u = unitNameById[String(row.birim_id)] || 'Birim'
    return `Birim · ${u}`
  }
  const c = companyNameById[String(row.ana_sirket_id)] || 'Şirket'
  return `Şirket · ${c}`
}
