...
import { isTaskVisibleNow } from '../lib/taskVisibility'
...
import { partitionHomeTasksWithHidden } from '../lib/taskHomeHidden'
import {
  addHomeForceShowTask,
  fetchHomeForceShowTaskIds,
  removeHomeForceShowTask,
} from '../lib/taskHomeHiddenApi'
...
export function useCubicleHomeData() {
  ...
  const applyEnrichment = useCallback(
    async (baseTasks, enrichGen) => {
      ...
      try {
        ...
        if (__DEV__) console.warn('[useCubicleHomeData] applyEnrichment', e)
      } finally {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const metaPromise = enrichScopeWithJunctionPersonelIds(supabase, scopePayload).then((scope) =>
        Promise.all([
          scopePersonelQuery(supabase, scope, 'id,ad,soyad,email,birim_id').is('silindi_at', null),
      ...
      if (personelId && timeVisible.some(taskNeedsWorkActionEnrich)) {
        enrichTasksWithWorkActions(supabase, timeVisible, personelId)
          ...
          .catch((e) => {
            if (__DEV__) console.warn('[useCubicleHomeData] enrichTasksWithWorkActions', e)
          })
      }
    } catch (e) {
