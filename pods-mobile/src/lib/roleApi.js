...
function formatRoleSaveError(e) {
  ...
  if (code === '42501' || /permission denied|row-level security|policy|yetkiniz yok|kapsam/i.test(msg)) {
    return `Rol kaydedilemedi: ${msg || 'veritabanı izni yok'}`
  }
  return msg || 'Rol kaydedilemedi'
}

export async function saveRollerRole({ rolId = null, rolAdi, anaSirketId = null, yetkiler }) {
  const payload = {
    p_rol_id: rolId || null,
  ...
  if (!saved?.id) {
    throw new Error('Kayıt uygulanmadı.')
  }
