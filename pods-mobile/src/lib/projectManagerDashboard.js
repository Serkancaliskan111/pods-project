...
import { formatDurationTr } from './managerReportSummary.js'
import { parseProjectDate } from './projectGanttUtils.js'
...
export function buildProjectUrgentAlerts(tasks, range) {
  ...
  if (noOperational > 5) {
    items.push({
      ...
      detail: `${noOperational} planlama görevi henüz operasyonel göreve bağlanmadı.`,
      count: noOperational,
      action: 'operational',
    })
  }
...
export function labelForProjectKpiDateFilter(key) {
  return PROJECT_KPI_DATE_FILTERS.find((f) => f.key === key)?.label || 'Son 30 gün'
}

export const PROJECT_REPORT_SCOPE_OPTIONS = PROJECT_KPI_DATE_FILTERS.filter((f) => f.key !== 'custom').map(
  (f) => ({ value: f.key, label: f.label }),
)

export function labelForProjectReportScope(scope) {
  return PROJECT_REPORT_SCOPE_OPTIONS.find((o) => o.value === scope)?.label || 'Son 30 gün'
}

/** Ana sayfa rapor özeti kartları ile uyumlu proje metrikleri */
export function buildProjectReportSummary(tasks = []) {
  const base = getActiveProjectTasks(tasks)
  const total = base.length
  const completed = base.filter((t) => t.durum === PROJECT_TASK_STATUS.DONE).length
  const completionRate = total > 0 ? Math.round((completed / total) * 100) : 0

  const urgentJobs = base.filter((t) => t.acil)
  const urgentDone = urgentJobs.filter((t) => t.durum === PROJECT_TASK_STATUS.DONE).length
  const urgentCompletionRate =
    urgentJobs.length > 0 ? Math.round((urgentDone / urgentJobs.length) * 100) : 0

  const doneWithDue = base.filter((t) => t.durum === PROJECT_TASK_STATUS.DONE && t.bitis_tarihi)
  const onTimeCount = doneWithDue.filter((t) => {
    const due = endOfDay(parseProjectDate(t.bitis_tarihi) || new Date(0))
    const doneAt = new Date(t.guncelleme_at || t.olusturulma_at || 0).getTime()
    return !Number.isNaN(doneAt) && doneAt <= due.getTime()
  }).length
  const onTimeCompletionRate =
    doneWithDue.length > 0 ? Math.round((onTimeCount / doneWithDue.length) * 100) : 0

  const completionDeltas = base
    .filter((t) => t.durum === PROJECT_TASK_STATUS.DONE)
    .map((t) => {
      const start = new Date(t.olusturulma_at || 0).getTime()
      const end = new Date(t.guncelleme_at || t.olusturulma_at || 0).getTime()
      return end > start ? end - start : null
    })
    .filter((ms) => ms != null && ms > 0)
  const avgMs = completionDeltas.length
    ? completionDeltas.reduce((sum, ms) => sum + ms, 0) / completionDeltas.length
    : 0

  const linked = base.filter((t) => t.bagli_is_id).length
  const efficiencyScore = total > 0 ? Math.round((linked / total) * 100) : 0

  const pending = base.filter((t) => t.durum !== PROJECT_TASK_STATUS.DONE)
  const opPending = pending.filter((t) => !t.bagli_is_id).length
  const resubmissionRate = pending.length > 0 ? Math.round((opPending / pending.length) * 100) : 0

  return {
    completionRate,
    urgentCompletionRate,
    avgApprovalLabel: avgMs > 0 ? formatDurationTr(avgMs) : '—',
    efficiencyScore,
...
export function filterProjectTasksForReportScope(tasks, scope) {
  const range = resolveProjectDateRange(scope || '30d')
  return filterProjectTasksByDateRange(tasks, range)
}
