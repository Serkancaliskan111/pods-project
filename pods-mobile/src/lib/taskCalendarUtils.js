...
export function resolveCalendarRange(viewMode, anchorDate) {
  ...
  if (viewMode === CALENDAR_VIEW.WEEK || viewMode === CALENDAR_VIEW.GANTT) {
...
export function getTaskSpan(task) {
  const start =
    ...
    parseTaskDate(task?.baslangic_tarihi) ||
    parseTaskDate(task?.gorunur_tarih) ||
    parseTaskDate(task?.olusturulma_at) ||
    parseTaskDate(task?.created_at)
  let end =
    parseTaskDate(task?.son_tarih) ||
    parseTaskDate(task?.bitis_tarihi) ||
    start
...
export function formatCalendarRangeLabel(start, end, viewMode) {
  ...
  if (viewMode === CALENDAR_VIEW.GANTT) {
    const a = start.toLocaleDateString('tr-TR', { day: 'numeric', month: 'short' })
...
/** Görevleri sorumlu personel gruplarına ayırır — takvim / proje Gantt ekip modu */
export function buildAssigneeGroupedGanttRows(tasks, staffMap) {
  ...
  for (const assigneeId of personIds) {
    ...
    const label =
      assigneeId === '__none__' ? 'Atanmamış' : staffDisplayName(staffMap, assigneeId)
    rows.push({
      kind: 'person',
...
export function buildGanttRows(tasks, filter, personelId, staffMap) {
  ...
  if (filter === CALENDAR_FILTER.TEAM) {
    return buildAssigneeGroupedGanttRows(list, staffMap)
  }
