...
/**
 * Kurumsal duyuru yayınlama (mobil ana sayfa ile aynı kural).
 * Yönetim kapsamı + görev atama / onay / rapor / personel yönetiminden en az biri.
 */
export function canCreateAnnouncement(perms, isSystemAdmin, personel = null) {
  if (isSystemAdmin) return true
  if (!personel?.id || !personel?.ana_sirket_id) return false
  if (!hasManagementPrivileges(perms, personel)) return false
  const flat = normalizeRolePermissions(perms)
  return (
    canAssignTask(perms, false, personel) ||
    canApproveTask(flat) ||
    isPermTruthy(flat, 'view_reports') ||
    canManageStaff(flat, false)
  )
}
