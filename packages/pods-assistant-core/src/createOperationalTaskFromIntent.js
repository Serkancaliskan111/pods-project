import { GOREV_TURU } from './deps/zincirTasks.js'
import { TASK_STATUS } from './deps/taskStatus.js'
import { formatTaskTitleCase } from './deps/formatTaskTitle.js'
import { normalizeOperasyonelOpts } from './deps/projectTaskOperasyonel.js'
import { linkProjectTaskToOperational } from './deps/projectApi.js'
import { validateIntent } from './validateIntent.js'
import { buildSiraliPayload } from './buildSiraliPayload.js'
import { resolveAiTaskSchedule, resolveBirimIdForTask } from './resolveAiTaskSchedule.js'
import { notifyTasksChanged } from './notifyTasksChanged.js'
import { ensureFutureScheduleFlags } from './premiumOrchestratorUtils.js'

function logTrace(trace, step, data) {
  if (trace?.log) trace.log(step, data)
}
...
export async function createOperationalTaskFromIntent({
  ...
  const context = { canAssignTask, templates, personnel }
  logTrace(debugTrace, 'create.validate.start', { mode: intent?.mode })
  intent = ensureFutureScheduleFlags(intent || {})
  const { ready, gaps, intent: resolvedIntent } = validateIntent(intent, context)
  if (!ready) {
    logTrace(debugTrace, 'create.validate.fail', { gaps })
    throw new Error(`Eksik bilgiler: ${gaps.join(', ')}`)
  }
  intent = resolvedIntent
  logTrace(debugTrace, 'create.validate.ok', { mode: intent.mode, title: intent.baslik })
  ...
  logTrace(debugTrace, 'create.assigner', { atayanPersonelId, anaSirketId })
  if (!atayanPersonelId) {
  ...
  let { baslamaIso, sonIso, gorunurIso } = resolveAiTaskSchedule(intent)
  if (!sonIso && intent.bitis) {
    sonIso = mergeDateTime(intent.bitis, intent.bitisSaat || '18:00')
  }
  if (!sonIso && intent.bitisSaat) {
    const today = new Date()
    const ymd = `${today.getFullYear()}-${String(today.getMonth() + 1).padStart(2, '0')}-${String(today.getDate()).padStart(2, '0')}`
    sonIso = mergeDateTime(intent.bitis || ymd, intent.bitisSaat)
  }
  ...
  if (!canAssignTask) {
    ...
    const birimForInsert = resolveBirimIdForTask({
      assigneeId: firstWorkerId,
      personnel,
      assignerPersonel: personel,
      intent,
    })
    ...
    logTrace(debugTrace, 'create.zincir', {
      tur,
      firstWorkerId,
      zincirGorevSira,
      atayanPersonelId,
      selfBlocked: String(firstWorkerId) === String(atayanPersonelId) && tur !== GOREV_TURU.ZINCIR_ONAY,
    })
    ...
    const rawAssigneeIds =
      intent.assigneeIds?.length
        ...
        : intent.personId
          ...
          : []
    const assigneeIds = rawAssigneeIds.filter((id) => String(id) !== String(atayanPersonelId))

    logTrace(debugTrace, 'create.assignees', {
      mode,
      tur,
      rawAssigneeIds,
      assigneeIds,
      filteredSelf: rawAssigneeIds.length !== assigneeIds.length,
    ...
    const payloads = assigneeIds.map((aid) => {
      const birimId = resolveBirimIdForTask({
        assigneeId: aid,
        personnel,
        assignerPersonel: personel,
        intent,
      ...
      if (!birimId) {
        throw new Error('Atanan personelin birimi tanımlı değil; görev listesinde görünmesi için birim zorunlu.')
      }
      return {
        ...payloadCommon,
    ...
    logTrace(debugTrace, 'create.insert.ok', { rowCount: rows.length, taskIds: rows.map((r) => r.id) })
  }
