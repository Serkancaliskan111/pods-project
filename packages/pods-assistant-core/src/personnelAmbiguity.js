...
export function formatAmbiguityQuestion(amb) {
  const opts = (amb.candidates || []).map((p) => personDetail(p)).join(' · ')
  return `Birden fazla **${amb.token}** var. Hangisi? (${opts})`
}
...
export function refreshPersonnelAmbiguities(intent, text, roster = []) {
  const fromText = findAmbiguousFirstNames(text, roster)
  const existing = intent?.pendingAmbiguities || []

  if (fromText.length) {
    return { ...intent, pendingAmbiguities: fromText }
  }

  if (!existing.length) {
    return { ...intent, pendingAmbiguities: [] }
  }

  const resolved = fullNameMatchesInText(text, roster)
  if (!resolved.length) {
    return { ...intent, pendingAmbiguities: existing }
  }

  const resolvedIds = new Set(resolved.map((p) => String(p.id)))
  const remaining = existing
    .map((amb) => ({
      ...amb,
      candidates: (amb.candidates || []).filter((p) => !resolvedIds.has(String(p.id))),
    }))
    .filter((amb) => amb.candidates?.length >= 2)

  return { ...intent, pendingAmbiguities: remaining }
}
